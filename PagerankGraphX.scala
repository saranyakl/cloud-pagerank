...
object PagerankGraphX {
  ...
  def printTopPages(wikiData: String, masterUrl: String, topC: Int, iters: Int, path: String, sc: SparkContext): Graph[(Double, String), Double] = {
    ...
    write("The total number of vertices is: " + vertices.count(), path)
    ...
    write("The total number of edges is: " + edges.count(), path)
    ...
    write("Total Time taken to build graph is: " + (System.currentTimeMillis() - t1), path)
    ...
    write("Total time taken at the end: " + (System.currentTimeMillis() - t1 ), path)
    titleAndPrGraph
  }
